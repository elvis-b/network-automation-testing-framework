...
async def get_alerts(
    ...
    cursor = alerts_collection.find(query).sort(
        [("acknowledged", 1), ("timestamp", -1)]  # Unacknowledged first  # Newest first
    )
...
async def get_alert(alert_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid alert ID format"
        )
...
async def create_alert(
    alert: AlertCreate, current_user: dict = Depends(get_current_user)
):
    """
    Create a new alert.
    ...
    """
    logger.info(
        f"Creating alert for device {alert.device_id} - user: {current_user.get('sub')}"
    )
...
async def acknowledge_alert(
    ...
    return AcknowledgeResponse(
        id=alert_id, acknowledged=True, acknowledged_by=username, acknowledged_at=now
    )
...
async def resolve_alert(alert_id: str, current_user: dict = Depends(get_current_user)):
    ...
    now = datetime.utcnow()
    update_data = {"resolved": True, "resolved_at": now}
    ...
    await alerts_collection.update_one({"_id": object_id}, {"$set": update_data})
