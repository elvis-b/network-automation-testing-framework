...
from app.models.device import (
    ...
    DeviceListResponse, DeviceStatus, DeviceType
)
...
async def get_devices(
    ...
    device_type: Optional[DeviceType] = Query(None, alias="type", description="Filter by device type"),
...
async def update_device(
    ...
    update_data = {
        k: v for k, v in device_update.model_dump().items() 
        if v is not None
    }
    ...
    if "ip_address" in update_data and update_data["ip_address"] != existing["ip_address"]:
