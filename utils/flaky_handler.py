    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if wait_for_stability(
                verification_func, timeout=verification_timeout, stable_count=1
            ):
                return True
            ...
            logger.warning(f"Action failed (attempt {attempt}/{max_attempts}): {e}")

    raise AssertionError(f"UI action did not succeed after {max_attempts} attempts")
