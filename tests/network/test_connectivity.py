...
def test_device_reachability(network_device):
    ...
    assert "Cisco" in output or "IOS" in output, "Output should indicate Cisco device"
...
def test_connection_timeout_handling(testbed):
    ...
    logger.info(
        "Connection timeout handling test - framework should skip gracefully if unreachable"
    )
