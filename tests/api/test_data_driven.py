...
def test_create_valid_devices(api_client, api_base_url, device_data):
    ...
    test_device["ip_address"] = (
        f"10.250.{uuid.uuid4().int % 250}.{1 + (uuid.uuid4().int % 253)}"
    )

    response = api_client.post(f"{api_base_url}/devices", json=test_device)
...
@pytest.mark.parametrize(
    "invalid_data", INVALID_DEVICES, ids=lambda d: d["description"]
)
def test_create_invalid_devices_fails(api_client, api_base_url, invalid_data):
    ...
    - Empty values
    """
    response = api_client.post(f"{api_base_url}/devices", json=invalid_data["data"])
...
def test_device_status_transitions(api_client, api_base_url, transition):
    ...
    update_response = api_client.put(
        f"{api_base_url}/devices/{device_id}", json={"status": transition["to_status"]}
    )
...
@pytest.mark.api
@pytest.mark.parametrize("scenario", LOGIN_SCENARIOS, ids=lambda s: s["description"])
...
@pytest.mark.api
@pytest.mark.parametrize(
    "filter_config", SEARCH_FILTERS, ids=lambda f: f["description"]
)
def test_device_filtering(api_client, api_base_url, filter_config):
    ...
    response = api_client.get(f"{api_base_url}/devices", params=params)
...
@pytest.mark.api
@pytest.mark.parametrize(
    "device_data", EXTERNAL_DEVICE_DATA, ids=lambda d: d.get("name", "device")
)
def test_devices_from_external_file(api_client, api_base_url, device_data):
    ...
    test_device["ip_address"] = (
        f"10.251.{uuid.uuid4().int % 250}.{1 + (uuid.uuid4().int % 253)}"
    )
...
class TestBulkOperations:
    ...
    def test_create_multiple_devices_sequentially(self, api_client, api_base_url):
        ...
        for device_data in VALID_DEVICES[:3]:
            ...
            test_device["ip_address"] = (
                f"10.252.{uuid.uuid4().int % 250}.{1 + (uuid.uuid4().int % 253)}"
            )
    ...
    def test_create_n_devices(self, api_client, api_base_url, count):
        ...
        assert (
            created_count == count
        ), f"Expected to create {count} devices, created {created_count}"
