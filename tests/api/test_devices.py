...
def test_get_devices_pagination(api_base_url, api_client):
    ...
    response = api_client.get(
        f"{api_base_url}/devices", params={"limit": 2, "offset": 0}
    )
...
def test_create_device(
    api_base_url, api_client, unique_device_data, cleanup_test_devices
):
    """
...
def test_update_device(
    ...
    create_response = api_client.post(
        f"{api_base_url}/devices", json=unique_device_data
    )
...
def test_update_device_not_found(api_base_url, api_client):
    ...
    response = api_client.put(
        f"{api_base_url}/devices/{fake_id}", json={"location": "New Location"}
    )
